namespace AnimalKingdom.root.Metazoa.Chordata.Actinopteri.Stomiiformes.Stomiidae.Idiacanthus.unclassified_Idiacanthus_in_ray_finned_fishes;

/// <summary>
/// Interface defining characteristics of unclassified_Idiacanthus_in_ray_finned_fishes (no rank).
/// </summary>
public interface Iunclassified_Idiacanthus_in_ray_finned_fishes
{
    /// <summary>Gets the description of this taxon.</summary>
    string Description { get; }
}
