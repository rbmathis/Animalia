namespace AnimalKingdom.root.Metazoa.Chordata.Actinopteri.Siluriformes.Trichomycteridae.Vandellia.unclassified_Vandellia_in_ray_finned_fishes;

/// <summary>
/// Interface defining characteristics of unclassified_Vandellia_in_ray_finned_fishes (no rank).
/// </summary>
public interface Iunclassified_Vandellia_in_ray_finned_fishes
{
    /// <summary>Gets the description of this taxon.</summary>
    string Description { get; }
}
